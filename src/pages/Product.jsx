    ...
    if (selectedSize !== "") {
      ...
      if (currentIndex >= 0) {
        ...
        setCookies('cart', cart, { path: '/' });
          ...
          setAmount(amount);
         alert(`there are only ${product.stock} pecies now`)
        }
      } else {
        setCart([...cart, {
        ...
        <Box display='flex' justifyContent='center'>
          <SimpleGrid width={1200} columns={{ base: 1, md: 2 }} >
            ...
            <Box p={3} maxWidth={600} >
              ...
              <Text mt={5} mb={3} fontSize={20} fontWeight={400} color='facebook.500' >Stock :  
              <b className={product.stock < 1 ? "redColor" : "greenColor"}>
              {product.stock < 1 ? "OutOfStock" : "InStock"}
              {product.stock >= 1 && <span style={{fontSize:"10px"}}>{`${" "}There are ${product.stock} pieces` }</span>}
            </b> </Text>
