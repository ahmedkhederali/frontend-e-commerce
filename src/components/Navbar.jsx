  ...
  return (
    <Box
      ...
      >
        ...
        <Searchbar />
        <Box display={{ base: 'none', md: 'flex' }} alignItems='center' px={2} style={{alignItems: "stretch"}} >
          <Box
            color="facebook.700"
            display='flex'
            flexDirection='column'
          ...
          >
            {
              currentUser && !admin &&
                <Menu isOpen={open}>
                  <Icon fontSize={30} color="facebook.700" as={Person} />
                  <Text color="facebook.700" fontWeight={500} >Account</Text>
                  <MenuButton />
                  <MenuList >
            ...
            }
            {
               ...
               <>
                 <Icon fontSize={30} color="facebook.700" as={Person} />
                 <Text color="facebook.700" fontWeight={500} >{t("Login")}</Text>
               </>
            }
            {
              admin && currentUser &&
              <Menu isOpen={open}>
                  <Icon fontSize={30} color="facebook.700" as={Person} />
                  <Text color="facebook.800" fontWeight={500} >Admin</Text>
                  <MenuButton />
                  <MenuList >
          ...
          >
            <Icon fontSize={30} color="facebook.700" as={Favorite} />
            <Text color="facebook.700" fontWeight={500} >{t("Favorites")}</Text>
          </Box>
          <Box
           color="facebook.700"
            display='flex'
          ...
          >
            <Icon fontSize={30} color="facebook.700" as={ShoppingCart} />
            <Text color="facebook.700" fontWeight={500} >{itemCount > 0 ? `Cart (${itemCount})` :t('Cart') }</Text>
          </Box>
          <Box
            color="facebook.700"
            display='flex'
